	...
	class Booked_Admin_AJAX {
		...
		public function booked_admin_add_custom_timeslot(){
			...
			if (isset($_POST['startTime']) && isset($_POST['endTime']) && isset($_POST['count']) && isset($_POST['currentTimes'])):
				...
				$current_times_details[$startTime.'-'.$endTime]['price'] = $price;
		...
		public function booked_admin_add_custom_timeslots(){
			...
			if (isset($_POST['startTime']) && isset($_POST['endTime']) && isset($_POST['interval']) && isset($_POST['count']) && isset($_POST['time_between'])):
				...
				do {
					...
					$current_times_details[$startTime.'-'.$newStartTime]['price'] = $price;
		...
		public function booked_admin_add_timeslots(){
			...
			if (isset($_POST['day']) && isset($_POST['startTime']) && isset($_POST['endTime'])):
				...
				do {
					...
					$booked_defaults[$day.'-details'][$startTime.'-'.$newStartTime]['price'] = $price;
		...
		public function booked_admin_add_timeslot(){
			...
			if (isset($_POST['day']) && isset($_POST['startTime']) && isset($_POST['count'])):
				...
				$booked_defaults[$day.'-details'][$startTime.'-'.$endTime]['price'] = $price;
		...
		public function booked_admin_delete_appt(){
			...
			if (isset($_POST['appt_id'])):
				...
				$appt_note = isset( $_POST['appt_note'] ) && ! empty( $_POST['appt_note'] ) ? esc_html( $_POST['appt_note'] ) : '';
				update_post_meta( $appt_id, '_appointment_note', $appt_note );
		...
		public function booked_admin_approve_appt(){
			...
			if (isset($_POST['appt_id'])):
				...
				$appt_id = $_POST['appt_id'];
				$appt_note = isset( $_POST['appt_note'] ) ? esc_html( $_POST['appt_note'] ) : '';
			...
			if (isset($_POST['json_array'])):
				...
				if (!empty($timeslots)):
					...
					foreach ($timeslots as $timeslot => $count):
						...
						$count_ttl = ($count>1000?'Unlimited':$count);
						...
						echo '<span class="count"><em>'.$count_ttl.'</em> ' . _n('Space Available','Spaces Available',$count_ttl,'wyzi-business-finder') . '</span>';

						if($count<=1000)
							echo '<span class="changeCount add" data-count="1"><i class="booked-icon booked-icon-plus-circle"></i></span></span>';
						...
						if (  !empty( $timeslots_detailed[$timeslot] ) ) {
							...
							if ( !empty($timeslots_detailed[$timeslot]['title']) ) {
								echo "<span class=\"title\">" . stripcslashes($timeslots_detailed[$timeslot]['title']) . '</span>';
							}
							if ( !empty($timeslots_detailed[$timeslot]['price']) ) {
								$price = esc_html($timeslots_detailed[$timeslot]['price']);
