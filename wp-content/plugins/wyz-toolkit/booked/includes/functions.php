			...
			for($i = 1; $i <= $rows; $i++){
				...
				for($x = 1; $x <= 7; $x++){
					...
					if ($hide_weekends && $day_of_week == 6 || $hide_weekends && $day_of_week == 7):
						...
						$spaces_available = $appointments_left > 1000 ? esc_html__('Unlimited Spaces','wyzi-business-finder') : sprintf( _n('%d Available','%d Available',$appointments_left,'wyzi-business-finder'),$appointments_left);

						$html = '<td data-date="'.$check_year.'-'.$check_month.'-'.$date.'" class="'.implode(' ',$classes).'">';
						$html .= '<span class="date'.(!$hide_available_count && $appointments_left > 0 && !in_array('prev-date',$classes) && !in_array('blur',$classes) ? ' tooltipster" title="'.$spaces_available : (!$hide_available_count && !$appointments_left && $booked_pa_active && !in_array('prev-date',$classes) && !in_array('blur',$classes) ? ' tooltipster" title="'.esc_html__('None Available','wyzi-business-finder').'"' : '')).'"><span class="number">'. $date .'</span></span>';
						$html .= '</td>';
				...
				*/
				...
				if ($spots_available && $available || !$hide_unavailable_timeslots):
					...
					if ( !empty( $todays_defaults_details[$timeslot] ) ) {
						...
						$price = !empty($todays_defaults_details[$timeslot]['price']) ? $todays_defaults_details[$timeslot]['price'] : '0';
					}
					...
					if ($hide_unavailable_timeslots && !$available):
						...
						$html = '<div class="timeslot bookedClearFix'.($title && $only_titles == true ? ' booked-hide-time' : '').($hide_available_count || !$available ? ' timeslot-count-hidden' : '').(!$available ? ' timeslot-unavailable' : '').($title ? ' has-title ' : '').'">';
							...
							$html .= '<span class="timeslot-time'.($public_appointments ? ' booked-public-appointments' : '').'">';
								...
								$available_spc = (1000 > $spots_available ? sprintf(_n('%d space available','%d spaces available',$spots_available,'wyzi-business-finder'),$spots_available) : esc_html__( 'Unlimited Space Available', 'wyzi-business-finder' ) );
								$html .= '<span class="timeslot-range"><i class="booked-icon booked-icon-clock"></i>&nbsp;&nbsp;' . $timeslotText . '</span>';
								if (!$hide_available_count): $html .= '<span class="spots-available'.($spots_available == 0 ? ' empty' : '').'">'.$available_spc.'</span>'; endif;
							...
							$html .= '</span>';
							$html .= '<span class="timeslot-people"><button data-price="'.$price.'" data-calendar-id="'.$calendar_id.'" data-title="'.esc_attr($title).'" data-timeslot="'.$timeslot.'" data-date="'.$date.'" class="new-appt button"'.(!$spots_available || !$available ? ' disabled' : '').'>'.( $title ? '<span class="timeslot-mobile-title">'.esc_html($title).'</span>' : '' ).'<span class="button-timeslot">'.apply_filters('booked_fe_mobile_timeslot_button',$timeslotText,$this_timeslot_timestamp,$timeslot,$calendar_id).'</span>'.apply_filters('booked_button_book_appointment', '<span class="button-text">' . $button_text . '</span>' . ( !$hide_available_count ? '<span class="spots-available' . ( $spots_available == 0 ? ' empty' : '' ) . '">' . $available_spc . '</span>' : '' ) ).'</button></span>';
						$html .= '</div>';
					endif;
...
function booked_profile_content_edit(){return;
