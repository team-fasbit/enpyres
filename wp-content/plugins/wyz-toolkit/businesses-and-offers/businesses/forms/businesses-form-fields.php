...
function wyz_after_group_row( $name ) {
	...
	for( $i=0; $i<3; $i++ ) {
		...
		$data .= '<div><input '.( ( $val == $values[ $i ]['key'] || ( empty( $val ) && $i == 2 ) )?'checked ':'').'id="' . $prefix . "open_close_" . $name . "_status$i" . '" type="radio" name="' . $prefix . 'open_close_' . $name . '_status' . '" value="' . $values[ $i ]['key'] . '"/><label for="' . $prefix . 'open_close_' . $name . "_status$i" . '" >' . $values[ $i ]['value'] . '</label></div>';
	}
	$data .= '</div>';
	return $data;
}
