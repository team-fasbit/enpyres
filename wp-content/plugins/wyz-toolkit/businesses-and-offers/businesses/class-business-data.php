...
class WyzSingleBusinessData{
	...
	public function __construct( $template_type, $business_path, $business_url ) {
		...
		$this->responsive = 1 == $this->template_type ? ( function_exists( 'wyz_get_option' ) && 'on' === wyz_get_option( 'resp' ) ? 'col-lg-8 col-md-7 col-xs-12' : 'col-xs-8') : '';
		$this->sidebar_responsive = ( function_exists( 'wyz_get_option' ) && 'on' === wyz_get_option( 'resp' ) ? 'col-lg-4 col-md-5 col-xs-12' : 'col-xs-4');
	...
	private function the_about($active,$urlid) {
		...
		<div class="tab-pane <?php echo $active; ?>" id="<?php echo $urlid;?>">
			<div class="business-about-area  <?php echo $this->responsive;?>">
				<?php echo wpautop( do_shortcode( get_post_meta( $this->business_id, 'wyz_business_description', true ) ) );?>
			</div>
			<?php $this->right_sidebar();?>
		</div>
		<?php 
	...
	private function the_internalMessaging($active,$urlid)
	{
		...
		<?php } else { ?>
			...
			<div class="tab-pane <?php echo $active; ?>" id="<?php echo $urlid;?>">
				<div class="busi-contact-wrapper login-string">
					<div class="head fix">
	...
	private function the_products($active,$urlid) {	 
		if ( $this->can_products ) {
			...
			<div class="tab-pane <?php echo $active; ?>" id="<?php echo $urlid;?>">
				...
				echo do_shortcode( "[wcmp_products ids='$posts' per_page='-1']" );?>
			</div>
			<?php
		}
	}
	...
	public function right_sidebar($getdynamicbar = false) {
		
		if ( $getdynamicbar ) { ?>
			
				<!-- Right Sidebar -->
				<div class="sidebar-container <?php echo ( function_exists( 'wyz_get_option' ) && 'on' === wyz_get_option( 'resp' ) ? 'col-md-3 col-xs-12' : 'col-xs-4');?>">
