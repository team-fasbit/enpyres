...
class WyzUserDashboard{
	...
	private function get_current_page() {
		...
		$this->current_page = apply_filters( 'wyz_user_dashboard_current_page', $this->current_page, $this->is_business_owner, $this->current_page_condition );
		$this->current_page_condition = apply_filters( 'wyz_user_dashboard_current_page_condition', $this->current_page_condition, $this->is_business_owner, $this->current_page );
	}
	...
	private function init_page_titles() {
		...
		$this->page_titles = apply_filters( 'wyz_user_dashboard_page_titles', $this->page_titles );
	}
	...
	public  function remove_unneeded_scripts () {
		if ( apply_filters( 'wyz_user_dashboard_skip_remove_unneeded_scripts', false ) )
			return;
		global $wp_scripts;
		global $wp_styles;
		...
		$scripts_to_keep = apply_filters( 'wyz_user_dashboard_scripts_to_keep', $scripts_to_keep );
		$styles_to_keep = apply_filters( 'wyz_user_dashboard_styles_to_keep', $styles_to_keep );
	...
	public function the_content() {
		if ( apply_filters( 'wyz_hide_user_dashboard_page', false, $this->current_page_condition, $this->current_page ) )
			return;
		$this->header();
		...
		<div class="container-fluid-full<?php echo ($this->is_mobile ? '' : ' sb-open');?>">
			<div class="row-fluid">
				...
				<div id="sidebar-overlay" class="sidebar-toggle<?php echo ($this->is_mobile ? '' : ' active');?>"></div>
		...
		<div class="navbar">
			<div class="navbar-inner">
				<div class="container-fluid main-header">
					...
					<a class="brand" href="<?php echo home_url();?>"><span><?php 
					$site_logo = wyz_get_option( 'header-logo-upload' );
					if ( '' !== $site_logo ) { ?>
		...
		<div id="sidebar-left" class="<?php echo ($this->is_mobile ? '' : 'in');?>">
			<div class="nav-collapse sidebar-nav">
				...
				<?php foreach ($this->nav_items as $key => $value) {
					...
					echo '<li' . ( isset( $value['class'] ) && ! empty( $value['class'] ) ? ' class="' . $value['class'] . '"' : '' ) . '><a href="'.($has_child?'dummy_holder':$value['link']).'"'.($has_child?' class="dropmenu"':'').'>';
	...
	private function page_content() {
		$path = apply_filters( 'wyz_user_dashboard_page_content', $this->tmpl_addr . $this->current_page . '.php', $this->current_page );
		if ( $this->current_page_condition && file_exists( $path ) )
			require( $path );
	}
	...
	private function setup_nav_links() {
		...
		$links['profile'] = array(
			'title' => esc_html__( 'Profile', 'wyzi-business-finder' ),
		...
		$this->nav_items = apply_filters( 'wyz_user_dashboard_menu_final_order', $final_order );
