...
class WyzHelpers extends WyzHelpersOverridden
{
	...
	public static function the_business_sidebar_1 ( $id, $days, $days_names, $days_arr, $author_id, $about, $address, $phone, $email, $website, $no_days_data ) {
		if ( method_exists( 'WyzHelpersOverride', 'the_business_sidebar_1') ) {
			return WyzHelpersOverride::the_business_sidebar_1( $id, $days, $days_names, $days_arr, $author_id, $about, $address, $phone, $email, $website, $no_days_data );
		}
		ob_start();
		?>
		<!-- Business Sidebar -->
		...
		<?php } 
			...
			<div class="business-sidebar <?php echo ( 'on' === wyz_get_option( 'resp' ) ? 'col-lg-4 col-md-5 col-xs-12' : 'col-xs-3');?>">
		<?php }
		if ( is_sticky() ) {
		...
		<?php if ( 'image' == get_option( 'wyz_business_header_content' ) &&  WyzHelpers::wyz_sub_can_bus_owner_do( $author_id,'wyzi_sub_business_show_map') ) { ?>
			<div class="sin-busi-sidebar">
		...
		?>
			...
			<?php if ( 'off' != get_option( 'wyz_business_claiming' ) && 'yes' != get_post_meta( $id, 'wyz_business_claimed', true ) ) {
	...
	public static function the_business_sidebar_2 ( $id, $days_names, $days_arr, $author_id, $address, $phone, $email, $website, $no_days_data ) {
		...
		<?php 
		if ( 'on' == get_option( 'wyz_switch_sidebars_single_bus','off' )) {
		...
		<div class="widget">
			...
			<h4 class="widget-title" style="float: left;"><?php esc_html_e( 'All Ratings', 'wyzi-business-finder' );?></h4>
			<!-- Rating Widget -->
		...
		if ( 'off' != get_option( 'wyz_business_claiming' ) && 'yes' != get_post_meta( $id, 'wyz_business_claimed', true ) ) {
			echo '<div class="widget"><a href="' . home_url( '/claim/?id=' ) . $id .'" class="light-blue-link wyz-prim-color-txt">' . esc_html__( 'Claim this Business', 'wyzi-business-finder' ) . '</a></div>';
		}
		if ( 'off' == get_option( 'wyz_switch_sidebars_single_bus','off' )) {
			if ( is_active_sidebar( 'wyz-single-business-sb' ) ) :
				dynamic_sidebar( 'wyz-single-business-sb' );
	...
	public static function social_links( $id ) {
		if ( method_exists( 'WyzHelpersOverride', 'social_links') ) {
			return WyzHelpersOverride::social_links( $id );
		}
		$social = array();
	...
	public static function get_post_thumbnail( $post_id, $post_type, $size = 'thumbnail', $attr = array() ) {
		...
		return '<img src="' . self::get_default_image( $post_type ) . '" ' . join(' ', array_map(function($key) use ($attr){
				if ( is_bool( $attr[ $key ] ) ) {
					return $attr[ $key ] ? $key : '';
				}
				return $key . '="' . $attr[ $key ] . '"';
	...
	public static function get_category_icon( $term_id ) {
		$cat_icn = wp_get_attachment_url( get_term_meta( $term_id, 'wyz_business_icon_upload', true ) );
		if ( ! $cat_icn || empty( $cat_icn ) )
