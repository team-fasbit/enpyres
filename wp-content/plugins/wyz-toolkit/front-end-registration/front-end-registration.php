	...
	<div class="login-form col-xs-12 fix">
		<form id="wyz_registration_form" enctype="multipart/form-data" class="wyz-form" method="POST">
			<div class="input-two">
				...
				<div class="input-box">
					<label for="wyz_user_new_pass_rep"><?php esc_html_e( 'Password Repeat', 'wyzi-business-finder' ); ?></label>
...
function wyz_login_member() {
	if ( isset( $_POST['wyz_user_login'] ) && wp_verify_nonce( $_POST['wyz_login_nonce'], 'wyz-login-nonce' ) ) {
		...
		if ( empty( $errors ) ) {
			...
			wp_redirect( apply_filters( 'wyz_after_login_redirect', home_url( '/user-account/' ), $user ) );
			exit;
		}
	}
}
		...
		if ( empty( $errors ) ) {
			...
			if ( 'on' == get_option( 'wyz_user_email_verification' ) ) {
				...
				if ( $new_user_id ) {
					...
					$url = apply_filters( 'wyz_after_login_redirect', home_url( '/user-account/' ), $user );
